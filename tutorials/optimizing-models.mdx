...
They use the same model `inputs` and `outputs` as regular model configs, which makes it easy to jump between training individual models and optimizing models — see the [Training Models](/tutorials/training-models#defining-model-inputs-and-outputs) tutorial for details on defining inputs/outputs.
...
<Card title="Simulating Models" icon="trending-up-down" href="/tutorials/simulating-models" horizontal>
