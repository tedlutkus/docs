...
<ParamField path="monitor_training" type="bool" default="True">
  Whether to display training progress in the terminal. Set to `False` for background training.
</ParamField>
...
onyx.train_model(
    ...
    dataset_name='example_train_data',
    training_config=training_config,
    monitor_training=False  # Returns immediately
)

# Monitor in the Engine Platform instead
```

## Training Output

With `monitor_training=True`, you'll see progress like:

```
Preparing to train model [example_model] using dataset [example_train_data].
Training started...
